...
public class StringFunctionExecutor implements FunctionExecutor{
	...
	@Comment("字符串多个替换")
	@Example("${string.multipleReplace(content,source,target)}")
	public static String multipleReplace(String content,List<String> sources,List<String> targets) throws Exception {
		String result = content;
		if (sources.size() != targets.size()){
			throw new Exception("The length of replace list and need to be replaced list are not same");
		}
		if (content == null){
			return null;
		}
		for (int i = 0; i < sources.size(); i++) {
			result = result.replace(sources.get(i), targets.get(i));
		}
		return result;
	}
	...
	@Comment("Match Regex")
	@Example("${string.regexMatch(srt,re)}")
	public static String regexMatch(String longInput, String regex){
		final Pattern compile = Pattern.compile(regex);
		final Matcher matcher = compile.matcher(longInput);
		if (matcher.find()){
